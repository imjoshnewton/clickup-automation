...
async function handleAutomationCompletion(taskId, automationBranchName, worktreePath) {
...
async function processTaskWithSDK(taskId, taskData) {
    ...
    try {
        ...
        try {
            // Remove existing worktree if it exists
            const removeWorktree = spawn("git", ["worktree", "remove", "--force", worktreePath], { cwd: REPO_PATH });
            await new Promise((resolve) => {
        ...
        // Use Claude Code SDK
        const prompt = buildTaskPrompt(taskId, taskName, taskDescription, customFields);
        ...
        try {
            ...
            if (result) {
                if (result.subtype === "success") {
                    ...
                    await logProgress(`Response: ${result.result?.substring(0, 100)}...`);
                    await logProgress("🎉 Task automation completed via Claude Code SDK!");
                    // Clean up the worktree after successful completion
                    await cleanupWorktree(worktreePath);
                }
                else {
                    await logProgress(`⚠️ SDK conversation failed: ${result.subtype}`, true);
                }
            }
            else {
...
async function copyConfigurationFiles(worktreePath) {
...
async function executeBashCommand(command, workingDirectory) {
    return new Promise((resolve, reject) => {
...
async function createFile(filePath, content, workingDirectory) {
...
async function readFile(filePath, workingDirectory) {
    const fullPath = path.join(workingDirectory, filePath);
    try {
...
async function executeClickUpUpdate(taskId, input) {
    try {
...
function buildTaskPrompt(taskId, taskName, taskDescription, customFields) {
...
async function cleanupWorktree(workingDirectory) {
    try {
